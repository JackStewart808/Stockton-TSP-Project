...
public class GraphBuilder {
    ...
    /**
     * Initialize distance + next matrices.
     * Before Floyd–Warshall runs, we set:
     *   - dist[i][i] = 0 (distance to itself)
     *   - dist[i][j] = INF for all others
     *   - next[i][i] = i
     *   - next[i][j] = -1 (unknown path)
     */
    ...
    /**
     * Fill edges based on the node's connections.
     * This builds the DIRECT edges graph.
     * Example: if A has a connection to B, we compute distance(A,B),
     * and set dist[A][B] to that distance.
     */
    ...
    public double[][] getDistMatrix() { return dist; }
